...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    // 1. Guardar en colección 'professionals' CON BLOQUEO INICIAL
    await setDoc(doc(db, "professionals", uid), {
      ...
      isAuthorized: false, // <--- AQUÍ ESTÁ EL CAMBIO IMPORTANTE
  ...
  <div style={{ maxWidth: '400px', margin: '50px auto', fontFamily: 'sans-serif' }}>
    ...
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
      ...
      <div>
        ...
        <input type="text" name="fullName" required value={formData.fullName} onChange={handleChange} style={{ width: '100%', padding: '8px' }} />
      </div>
      ...
      <div>
        ...
        <input type="text" name="license" required value={formData.license} onChange={handleChange} style={{ width: '100%', padding: '8px' }} />
      </div>
      ...
      <button
        type="submit"
        disabled={saving}
        style={{ marginTop: '20px', padding: '15px', backgroundColor: '#2196F3', color: 'white', border: 'none', cursor: 'pointer' }}
      >
