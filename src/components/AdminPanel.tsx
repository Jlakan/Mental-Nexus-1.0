...
export default function AdminPanel() {
  ...
  const handleEditClick = async (user: any) => {
    ...
    if (user.role === 'patient' || user.role === 'professional') {
      const collectionName =
        user.role === 'patient' ? 'patients' : 'professionals';
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (originalRole === 'patient' || originalRole === 'professional') {
        const collectionName =
          originalRole === 'patient' ? 'patients' : 'professionals';
  ...
  const handleDelete = async (uid: string, role: string) => {
    if (
      !window.confirm('¿Estás SEGURO? Se borrará de todas las bases de datos.')
    )
      return;
    try {
      ...
      if (role === 'professional')
        await deleteDoc(doc(db, 'professionals', uid));
      fetchAll();
    } catch (e) {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <input
            value={editForm.fullName || editForm.displayName || ''}
            onChange={(e) =>
    ...
    <div style={{ padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      >
        ...
        <button
          ...
          style={{
            padding: '10px',
            background: activeTab === 'requests' ? '#FF9800' : '#eee',
            color: activeTab === 'requests' ? 'white' : 'black',
          }}
        >
          🔔 Solicitudes ({pendingPros.length})
        </button>
        <button
          onClick={() => setActiveTab('catalog')}
          style={{
            padding: '10px',
            background: activeTab === 'catalog' ? '#333' : '#eee',
        ...
        <button
          ...
          style={{
            padding: '10px',
            background: activeTab === 'config' ? '#333' : '#eee',
      ...
      {activeTab === 'config' && (
        ...
        >
          ...
          >
            Enlace de Descarga / Invitación (Mental Nexus):
          </label>
          <input
