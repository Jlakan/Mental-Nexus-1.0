// Ruta: src/components/AdminPanel.tsx
...
export default function AdminPanel() {
  ...
  // Estados para Edición de Profesiones
  const [editingProfId, setEditingProfId] = useState<string | null>(null);
  const [editProfName, setEditProfName] = useState<string>('');
  ...
  const [userRoleFilter, setUserRoleFilter] = useState<
    'all' | 'patient' | 'pro' | 'admin'
  >('all');
  const [userProFilter, setUserProFilter] = useState<string>('all');
  const [userManualFilter, setUserManualFilter] = useState<
    'all' | 'app' | 'manual'
  >('all');
  ...
  const fetchProfessions = async () => {
    try {
      ...
      console.error('Error cargando profesiones:', e);
    }
  };
  ...
  const fetchUsers = async () => {
    try {
      ...
      const pros = prosSnap.docs.map(
        (d) => ({ uid: d.id, ...d.data() } as any)
      );
      setProfessionalsList(pros);

      const prosMap = new Map(
        pros.map((p) => [p.uid, p.fullName || p.displayName || 'Profesional'])
      );

      const registeredUsers = usersSnap.docs.map(
        (d) => ({ uid: d.id, ...d.data() } as any)
      );
      const manualPatients = patientsSnap.docs.map(
        (d) =>
          ({
            uid: d.id,
            ...d.data(),
            displayName:
              d.data().fullName || d.data().displayName || 'Sin Nombre',
  ...
  // --- LÓGICA: GESTIÓN DE PROFESIONES E ÍNDICES ---
  const handleAddProfession = async () => {
    if (!newProfForm.id || !newProfForm.name)
      return alert('Por favor llena ambos campos.');
    try {
      const batch = writeBatch(db);
      const profId = newProfForm.id;

      // 1. Crear documento principal de la profesión
      const profRef = doc(db, 'professions', profId);
      batch.set(profRef, {
      ...
      // 2. Crear documento shard_0 con el terapeuta Dummy
      const shardRef = doc(
        db,
        'professions',
        profId,
      ...
      const dummyId = `dummy_prof_${profId}`;
      const dummyProfile = {
        fullName: `Especialista Demo (${newProfForm.name})`,
        clinicName: 'Centro Nexus de Demostración',
        clinicCity: 'Ciudad Nexus',
        clinicAddress: 'Plaza Central 100',
        publicPhone: '0000000000',
        links: { maps: '', facebook: '', instagram: '' },
      };

      batch.set(shardRef, {
        professionals: {
          [dummyId]: dummyProfile,
        },
      });

      await batch.commit();
      setNewProfForm({ id: '', name: '' });
      alert('Especialidad e índice dummy creados correctamente.');
      fetchProfessions();
    } catch (error) {
      console.error(error);
      alert('Error al agregar especialidad e índice.');
    }
  };

  const handleSaveProfEdit = async (id: string) => {
    if (!editProfName.trim()) return alert('El nombre no puede estar vacío.');
    try {
      await updateDoc(doc(db, 'professions', id), { name: editProfName });
      setEditingProfId(null);
      setEditProfName('');
      fetchProfessions();
    } catch (error) {
      console.error(error);
      alert('Error al editar el nombre de la especialidad.');
    }
  };
  ...
  const handleDeleteProfession = async (id: string) => {
    if (
      !window.confirm(
        `¿Estás seguro de ELIMINAR por completo la profesión '${id}'? Esto borrará también su índice de búsqueda.`
      )
    )
      return;
    try {
      const batch = writeBatch(db);
      batch.delete(doc(db, 'professions', id, 'directory_shards', 'shard_0'));
      batch.delete(doc(db, 'professions', id));

      await batch.commit();
      alert('Profesión e índice eliminados permanentemente.');
      fetchProfessions();
    } catch (error) {
      console.error(error);
  ...
  // --- LÓGICA: INTELIGENCIA GLOBAL (Actualizada a nueva arquitectura) ---
  const handleLoadGlobalAnalytics = async () => {
    setLoadingAnalytics(true);
    try {
      const [plansSnap, archivesSnap] = await Promise.all([
        getDocs(collection(db, 'active_plans')),
        getDocs(collectionGroup(db, 'historical_archives')),
      ]);

      let allTasks: any[] = [];

      // Extraer de planes activos
      plansSnap.docs.forEach((d) => {
        const data = d.data();
        if (data.activeTasks) {
          Object.values(data.activeTasks).forEach((t: any) => {
            if (t.catalogId) allTasks.push(t);
          });
        }
      });

      // Extraer de archivos históricos
      archivesSnap.docs.forEach((d) => {
        const data = d.data();
        if (data.tasks) {
      ...
      // Normalizar para el motor
      const normalizedTasks = allTasks.map((t) => {
        let historyArray = [];
        if (Array.isArray(t.historyChecks)) historyArray = t.historyChecks;
        else if (Array.isArray(t.completionHistory))
          historyArray = t.completionHistory;
        else if (Array.isArray(t.completionLog)) historyArray = t.completionLog;
      ...
      const contentStats = analyzeCatalogBatch(normalizedTasks as any[]);
      let statsArray = Array.isArray(contentStats)
        ? contentStats
        : Object.values(contentStats);
      statsArray.sort((a: any, b: any) => b.usageCount - a.usageCount);

      const popStats = calculateAggregatedStats
        ? calculateAggregatedStats(usersList, normalizedTasks)
        : {};
      const correlations = calculateTagCorrelations
        ? calculateTagCorrelations(usersList)
        : [];
  ...
  const handleInitializeTagsSystem = async () => {
    ...
    try {
      ...
      if (!metadataSnap.exists()) {
        ...
        dbProfessions.forEach((p) => {
          initialMetadata[`${p.id}_version`] = 1;
        });
      ...
      professionsToInit.forEach((profId) => {
        batch.set(
          doc(db, 'tagsDictionaries', profId),
          { tags: [] },
          { merge: true }
        );
      });
    ...
    return usersList.filter((u) => {
      ...
      if (userProFilter !== 'all') {
        if (userProFilter === 'unassigned') {
          if (u.profName !== 'Sin asignar' && u.role === 'patient')
            return false;
          if (u.role !== 'patient' && !u.isManual) return false;
        } else {
          const isLinked =
            u.linkedProfessionalId === userProFilter ||
            (u.careTeam && u.careTeam[userProFilter]);
  ...
  const handleExportCSV = () => {
    if (!analyticsData || analyticsData.length === 0) {
      alert(
        'No hay datos para exportar. Carga la Inteligencia Global primero.'
      );
      return;
    }
    const headers = [
    ...
    const csvRows = analyticsData.map((row: any) => {
      ...
      return [
        ...
        safeTitle,
        row.usageCount,
        row.globalSuccessRate.toFixed(2),
        row.dropoutRate.toFixed(2),
        row.workloadImpact,
      ].join(',');
    });
    ...
    link.setAttribute(
      'download',
      `reporte_clinico_${new Date().toISOString().split('T')[0]}.csv`
    );
  ...
  const handleEditClick = async (user: any) => {
    ...
    let currentProId =
      ...
      (user.careTeam ? Object.keys(user.careTeam)[0] : '');
  ...
  const saveEdit = async () => {
    try {
      ...
      if (isPatient && editForm.linkedProfessionalId) {
        ...
        patientUpdates[`careTeam.${editForm.linkedProfessionalId}.status`] =
          'active';
        userUpdates.linkedProfessionalId = editForm.linkedProfessionalId;
        userUpdates[`careTeam.${editForm.linkedProfessionalId}.status`] =
          'active';
      }
      ...
      if (editingUser.isManual) {
        ...
        if (isPatient) {
          await setDoc(doc(db, 'patients', editingUser.uid), patientUpdates, {
            merge: true,
          });
        }
        if (editForm.role === 'pro') {
          ...
          await setDoc(
            proRef,
            { fullName: editForm.displayName, email: editForm.email },
            { merge: true }
          );
        }
      }
        ...
        <div style={{ display: 'flex', gap: '15px' }}>
          <button
            ...
            style={{
              ...
              border: 'none',
              borderRadius: '6px',
              cursor: isInitializingTags ? 'not-allowed' : 'pointer',
              fontWeight: 'bold',
              fontSize: '13px',
            }}
          >
              ...
              >
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEditForm({
                      ...editForm,
                      linkedProfessionalId: e.target.value,
                    })
                  }
                  style={{
                    ...modalInputStyle,
      ...
      {activeTab === 'config' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          >
            ...
            >
              Especialidades Clínicas e Índices
            </h3>
            ...
            >
              <div style={{ flex: 1, minWidth: '200px' }}>
                ...
                >
                  ID Interno (ej. terapia_ocupacional)
                </label>
                <input
              ...
              <div style={{ flex: 1, minWidth: '200px' }}>
                ...
                >
                  Nombre Visible (ej. Terapia Ocupacional)
                </label>
                <input
                  value={newProfForm.name}
                  onChange={(e) =>
                    setNewProfForm({ ...newProfForm, name: e.target.value })
                  }
                  style={{ ...modalInputStyle, marginTop: 0 }}
              ...
              <tbody>
                {dbProfessions.map((p) => (
                  <tr key={p.id} style={{ borderBottom: '1px solid #1E293B' }}>
                    <td style={{ padding: '12px', color: '#00E5FF' }}>
                      {p.id}
                    </td>
                    <td style={{ padding: '12px', fontWeight: 'bold' }}>
                      {editingProfId === p.id ? (
                        <div style={{ display: 'flex', gap: '5px' }}>
                          ...
                          />
                          <button
                            onClick={() => handleSaveProfEdit(p.id)}
                            style={{
                              background: '#22C55E',
                              color: 'white',
                              border: 'none',
                ...
                {dbProfessions.length === 0 && (
                  <tr>
                    ...
                    >
                      No hay profesiones registradas en la base de datos.
                    </td>
                  </tr>
                )}
              </tbody>
            ...
            <p
              style={{ color: '#64748B', fontSize: '12px', marginTop: '15px' }}
            >
              * El borrado permanente destruirá el documento de la profesión y
              su índice de directorio de forma irreversible.
            </p>
          </div>
        </div>
      )}
          ...
          <tbody>
            {pendingPros.map((p) => (
              <tr key={p.uid}>
                ...
                <td style={{ padding: '10px' }}>
                  <button onClick={() => handleAuthorize(p.uid)}>
                    Autorizar
                  </button>
                </td>
              </tr>
        ...
        >
          ...
          >
            ...
            <span style={{ color: '#94A3B8', fontSize: '14px' }}>
              Total de registros: {displayedUsers.length}
            </span>
          </div>
          <div
            style={{
          ...
          >
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (e.target.value === 'pro' || e.target.value === 'admin') {
                  setUserProFilter('all');
                  setUserManualFilter('all');
                }
              }}
              style={{
                padding: '10px',
            ...
            {(userRoleFilter === 'all' || userRoleFilter === 'patient') && (
              <select
                value={userProFilter}
                onChange={(e) => setUserProFilter(e.target.value)}
                style={{
                  padding: '10px',
                  borderRadius: '6px',
                  border: '1px solid #334155',
              ...
              >
                ...
                <option value="unassigned">⚠️ Pacientes Sin Asignar</option>
            ...
            {(userRoleFilter === 'all' || userRoleFilter === 'patient') && (
              <select
                ...
                onChange={(e) => setUserManualFilter(e.target.value as any)}
                style={{
                  padding: '10px',
                  borderRadius: '6px',
                  border: '1px solid #334155',
              ...
              >
                <option value="all">Cualquier Registro</option>
                <option value="app">📱 Usuarios de App</option>
                ...
                >
                  <span
                    style={{
                      ...
                      color: u.role === 'pro' ? '#64B5F6' : '#CE93D8',
                      fontSize: '11px',
                      fontWeight: 'bold',
                      border: `1px solid ${
                        u.role === 'pro' ? '#1976D2' : '#8E24AA'
                      }`,
                    }}
                  >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          >
            ...
            <button
              ...
              style={{
                padding: '10px 20px',
                background: analyticsData ? '#22C55E' : '#475569',
                color: 'white',
                border: 'none',
                borderRadius: '6px',
          ...
          {loadingAnalytics && (
            ...
            >
              🔄 Analizando millones de puntos de datos clínicos...
            </div>
          )}
            ...
            <>
              ...
              >
                ...
                <StatCard
                  ...
                  value={
                    usersList.filter((u) => u.role === 'patient' || u.isManual)
                      .length
                  }
                  icon="❤️"
                  colorHex="#22C55E"
                ...
                <StatCard
                  title="PROFESIONALES"
                  value={usersList.filter((u) => u.role === 'pro').length}
                  icon="🩺"
                  colorHex="#3B82F6"
                />
                <StatCard
                  ...
                  value={analyticsData.reduce(
                    (acc: any, curr: any) => acc + curr.usageCount,
                    0
                  )}
                  icon="📝"
                  colorHex="#F59E0B"
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                <div
                                  style={{
                                    width: `${stat.globalSuccessRate}%`,
                                    height: '100%',
                                    background:
                                      stat.globalSuccessRate > 70
                                        ? '#22C55E'
                                        : '#F59E0B',
                                    borderRadius: '3px',
                                  }}
                              ...
                              >
                                {stat.globalSuccessRate.toFixed(0)}%
                              </span>
                            </td>
                          </tr>
                        ))}
                    ...
                    {populationStats?.timeOfDayStats ? (
                      ...
                      >
                        <ProgressBar
                          label="Mañana (06-12)"
                          value={
                            populationStats.timeOfDayStats.morning?.completed ||
                            0
                          }
                          max={analyticsData.reduce(
                            (acc: any, curr: any) => acc + curr.usageCount,
                            0
                          )}
                          color="#F59E0B"
                        ...
                        <ProgressBar
                          label="Tarde (12-19)"
                          value={
                            populationStats.timeOfDayStats.afternoon
                              ?.completed || 0
                          }
                          max={analyticsData.reduce(
                            (acc: any, curr: any) => acc + curr.usageCount,
                        ...
                        <ProgressBar
                          label="Noche (19-06)"
                          value={
                            populationStats.timeOfDayStats.night?.completed || 0
                          }
                          max={analyticsData.reduce(
                            (acc: any, curr: any) => acc + curr.usageCount,
                            0
                    ...
                    {populationStats?.byCategory?.General?.dayDistribution ? (
                      ...
                      >
                        {['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'].map(
                          (day) => {
                            const val =
                              populationStats.byCategory.General
                                .dayDistribution[day] || 0;
                            const maxVal = Math.max(
                              10,
                              ...Object.values(
                                populationStats.byCategory.General
                                  .dayDistribution as Record<string, number>
                              )
                            );
                            ...
                            return (
                              ...
                              >
                                <div
                                  style={{
                                    height: `${h || 2}%`,
                                    background: val > 0 ? '#3B82F6' : '#F1F5F9',
                                    width: '24px',
                  ...
                  >
                    ...
                    >
                      <tr>
                        ...
                        >
                          Pacientes con esta combinación
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {tagCorrelations && tagCorrelations.length > 0 ? (
                        ...
                        <tr>
                          ...
                          >
                            Aún no hay suficientes datos para generar
