...
import { INITIAL_PLAYER_PROFILE } from '../utils/GamificationUtils'; // O '../utils/gameRules' si ya lo moviste
  ...
  try {
    ...
    if (formData.code.trim()) {
      ...
      if (!querySnapshot.empty) {
        ...
        // Creamos la entrada en el equipo de cuidado
        // Usamos la profesión (ej: 'psicologo') como clave para evitar duplicados del mismo tipo si así lo deseas,
        // o usamos el ID del profesional si permites varios psicólogos.
        // Aquí usaremos el ID de la profesión para mantener orden (1 nutri, 1 psico, etc).
        ...
        alert("El código ingresado no existe. Se creará la cuenta sin vinculación.");
        // No detenemos el registro, solo avisamos.
      }
    }
    ...
    await setDoc(doc(db, "patients", uid), {
      ...
      // NUEVA ESTRUCTURA: Equipo de Cuidado (Mapa)
      careTeam: initialCareTeam,
    
      // Perfil de Juego
      gamificationProfile: {
         ...INITIAL_PLAYER_PROFILE,
         wallet: { gold: 0, nexus: 0 } // Aseguramos estructura nueva
      },
