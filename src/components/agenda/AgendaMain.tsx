            ...
            <div key={key} style={{borderBottom:'1px solid #eee', padding:'12px', display:'flex', alignItems:'center', gap:'15px'}}>
              ...
              <div style={{flex:1}}>
                {slot.status === 'available' ? (
                  ...
                  <div onClick={() => handleReopenSlot(key)} style={{background:'#FFEBEE', padding:'10px', borderRadius:'6px', display:'flex', justifyContent:'space-between', cursor:'pointer', color: '#000'}}> <span>🚫 {slot.adminNotes}</span><span> ✕ </span> </div>
          ...
          <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.8)', display:'flex', justifyContent:'center', alignItems:'center', zIndex:2000}}>
            <div style={{background:'white', padding:'25px', borderRadius:'12px', width:'500px'}}>
              ...
              <div style={{maxHeight:'300px', overflowY:'auto', background:'#FFF8F8', border:'1px solid #FFCDD2', borderRadius:'8px', padding:'10px', margin:'15px 0'}}>
                {conflictList.map((c) => ( <div key={c.slotKey} style={{background:'white', padding:'10px', marginBottom:'10px', borderRadius:'6px', display:'flex', justifyContent:'space-between', alignItems:'center'}}> <div> <div style={{fontWeight:'bold', color: '#000'}}>{c.slotData.patientName}</div> <div style={{fontSize:'12px', color: '#333'}}>{dayjs(c.date).format('DD MMM')} - {c.slotData.time}</div> </div> <div style={{display:'flex', gap:'5px'}}> <button onClick={() => handleResolveConflictToWaitlist(c)} style={{background:'#7B1FA2', color:'white', border:'none', borderRadius:'4px', padding:'5px', fontSize:'11px', cursor:'pointer'}}>⏳ Espera</button> <button onClick={() => handleKeepConflict(c)} style={{background:'#eee', padding:'5px', fontSize:'11px', color: '#000', border: '1px solid #ccc', cursor:'pointer'}}>Mantener</button> </div> </div> ))}
              </div>
              <div style={{textAlign:'right'}}> <button onClick={() => setIsConflictModalOpen(false)} style={{marginRight:'10px', padding:'10px', cursor:'pointer'}}>Cancelar</button> <button onClick={finalizeEventSave} style={{background:'#D32F2F', color:'white', padding:'10px 20px', border:'none', borderRadius:'4px', cursor:'pointer'}}>Finalizar</button> </div>
            </div>
          </div>
        </ModalPortal>
      )}
    ...
    <div style={{
      ...
      <div style={{background:'white', padding:'25px', borderRadius:'12px', width:'350px', boxShadow: '0 10px 25px rgba(0,0,0,0.2)'}}>
        ...
        <form onSubmit={handleAddToWaitlist}>
          ...
          />
          <textarea 
            placeholder="Preferencia (Ej: Solo tardes)..." 
            value={formData.adminNotes} 
            onChange={e => setFormData({...formData, adminNotes: e.target.value})} 
            style={{width:'100%', marginTop:'15px', padding:'8px', borderRadius: '4px', border: '1px solid #ccc'}} 
          />
          <div style={{marginTop:'20px', textAlign:'right'}}> 
            <button type="button" onClick={() => setIsWaitlistFormOpen(false)} style={{marginRight:'10px', padding:'8px 15px', cursor:'pointer', border: '1px solid #ccc', borderRadius: '4px', color: '#000'}}>Cancelar</button> 
            <button type="submit" style={{padding:'8px 15px', background:'#9C27B0', color:'white', border:'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer'}}>Poner en espera</button> 
          </div>
        </form>
      </div>
    </div>
      ...
      {isWaitlistSelectorOpen && (
        <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', display:'flex', justifyContent:'center', alignItems:'center', zIndex:40}}>
          <div style={{background:'white', padding:'25px', borderRadius:'12px', width:'400px', maxHeight:'80vh', overflowY:'auto'}}>
            ...
            {waitlist.map(w => ( <div key={w.id} onClick={() => handleAssignFromWaitlist(w)} style={{padding:'10px', border:'1px solid #eee', marginBottom:'8px', borderRadius:'6px', cursor:'pointer', background:'#f9f9f9'}}> <div style={{fontWeight:'bold', color: '#000'}}>{w.patientName}</div> <div style={{fontSize:'12px', color:'#555'}}>{w.notes}</div> </div> ))}
      ...
      {confirmModal.isOpen && (
        <ModalPortal>
          <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', justifyContent:'center', alignItems:'center', zIndex:3000}}>
            <div style={{background:'white', padding:'25px', borderRadius:'12px', width:'350px', boxShadow: '0 10px 30px rgba(0,0,0,0.5)'}}>
              <h3 style={{color: '#000'}}>{confirmModal.title}</h3> <p style={{color: '#333'}}>{confirmModal.message}</p>
              <div style={{textAlign:'right', marginTop: '20px'}}> <button onClick={() => setConfirmModal({...confirmModal, isOpen:false})} style={{marginRight:'10px', padding:'8px 15px', cursor:'pointer'}}>Cancelar</button> <button onClick={confirmModal.onConfirm} style={{background:'#2196F3', color:'black', border:'none', padding:'8px 15px', borderRadius:'4px', cursor:'pointer', fontWeight:'bold'}}>Confirmar</button> </div>
            </div>
          </div>
        </ModalPortal>
      )}
