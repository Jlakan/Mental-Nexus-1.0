  ...
  const filteredAndSortedPatients = useMemo(() => {
    ...
    if (activeFilter === 'upcoming_appt') {
      ...
      result = result.filter((p) => {
        ...
        const daysSince =
          (now.getTime() - lastAtt.getTime()) / (1000 * 3600 * 24);
        return daysSince > 7;
      });
    }
    ...
    result.sort((a, b) => {
      if (sortBy === 'name_asc') {
        ...
        const timeA =
          apptA && new Date(apptA) > now ? new Date(apptA).getTime() : Infinity;
        const timeB =
          apptB && new Date(apptB) > now ? new Date(apptB).getTime() : Infinity;
