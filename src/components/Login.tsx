...
export default function Login() {
  ...
  const [viewState, setViewState] = useState<'waiting' | 'playing' | 'login'>(
    'waiting'
  );
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center relative overflow-hidden font-sans text-slate-200">
      ...
      {viewState === 'waiting' && (
        <div
          className="z-50 text-center animate-pulse cursor-pointer"
          onClick={startSystem}
        >
      ...
      ----------------------------------------------------- */}
      <div
        className={`absolute inset-0 z-40 bg-black transition-opacity duration-1000 ${
          viewState === 'playing'
      ...
      {/* Tarjeta de Login */}
      <div
        className={`w-full max-w-md z-30 transition-all duration-1000 transform ${
          viewState === 'login'
      ...
      {/* Footer Status */}
      <div
        className={`absolute bottom-4 text-[10px] text-slate-500 font-mono flex items-center gap-2 transition-opacity duration-1000 ${
          viewState === 'login' ? 'opacity-100' : 'opacity-0'
        }`}
      >
