...
export default function AssistantRegister() {
  ...
  const handleLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Nos agregamos a su lista de 'authorizedAssistants' en la BD (Perspectiva del Doctor)
      await updateDoc(doc(db, "professionals", profId), {
        authorizedAssistants: arrayUnion(auth.currentUser.uid)
      });

      // 3. Aseguramos que nuestro rol sea 'assistant' en la colección users (Perspectiva de Auth/User)
      await setDoc(doc(db, "users", auth.currentUser.uid), {
        role: 'assistant'
      }, { merge: true });

      // 4. NUEVO: Creamos/Actualizamos el perfil en la colección 'assistants' (Perspectiva del Asistente)
      await setDoc(doc(db, "assistants", auth.currentUser.uid), {
        uid: auth.currentUser.uid,
        email: auth.currentUser.email,
        displayName: auth.currentUser.displayName || 'Asistente sin nombre',
        linkedProfessionals: arrayUnion(profId), // ArrayUnion evita duplicados
        lastUpdated: new Date()
      }, { merge: true });

      setMsg(`✅ ¡Vinculado con éxito al Dr(a). ${profData.fullName}!`);

      // Recargamos para que el sistema actualice los permisos
