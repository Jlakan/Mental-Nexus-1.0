...
import { AtlasCard, AtlasButton, AtlasIcons } from './AtlasDesignSystem';
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (todaysData) {
            ...
            if (todaysData.timestamp)
              setLastSaved(new Date(todaysData.timestamp));
          }
        }
      } catch (error) {
  ...
  if (fetching) {
    return (
      <AtlasCard className="bg-slate-800/50 border-slate-700 flex justify-center py-10">
        <span className="text-cyan-500 font-mono animate-pulse text-xs">
          Desencriptando bitácora...
        </span>
      </AtlasCard>
    );
  }
  ...
  return (
    <AtlasCard className="bg-slate-800 border-cyan-900/50 shadow-lg">
      ...
      <p className="text-xs text-slate-400 mb-4">
        Registra tus pensamientos, avances o temas que desees tratar en tu
        próxima sesión. Tú decides quién tiene acceso a este bloque de memoria.
      </p>
      ...
      <div className="bg-slate-900/50 rounded-xl p-4 border border-slate-700/50 mb-6">
        ...
        {activeCareTeam.length === 0 ? (
          <p className="text-xs text-slate-500 italic">
            No tienes especialistas vinculados actualmente.
          </p>
        ) : (
            ...
            {activeCareTeam.map((prof: any) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="opacity-50 text-[9px] uppercase">
                    ({prof.professionType})
                  </span>
                </button>
              );
